   ...
   public class Z_Harag_MessageMap:NopEntityTypeConfiguration<Z_Harag_Message>
    {
        public Z_Harag_MessageMap()
        {
            ...
            this.HasOptional(m => m.Customer).WithMany(c => c.PostsMessages).HasForeignKey(m => m.CustomerId);
            this.HasOptional(m => m.Customer).WithMany(c => c.UserMessages).HasForeignKey(m => m.UserId);
        }
    }
}
